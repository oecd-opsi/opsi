...
function loadcss() {
	...
	wp_register_style( 'genstyle', get_template_directory_uri() . '/style.css', array(), filemtime( get_template_directory() . '/style.css' ), 'screen' );
...
function loadjs() {
	...
	// custom
	wp_register_script( 'custom_script', get_template_directory_uri() . '/js/custom.js', array( 'jquery' ), filemtime( get_template_directory() . '/js/custom.js' ) );
...
function opsi_post_types() {
	...
	nitro_taxonomy_creator( __( 'Innovation Badge', 'opsi' ), __( 'Innovation Badges', 'opsi' ), 'case', 'opsi', $badge_args );
	nitro_taxonomy_creator( __( 'Innovation Badge OpenGov', 'opsi' ), __( 'Innovation Badges OpenGov', 'opsi' ), 'case', 'opsi', $badge_args );
	...
	function meta_boxes() {
		...
		foreach ( $wp_post_types as $post_type ) {
			if ( $post_type->name == 'project' ) {
				...
				add_meta_box( 'postparentdiv', __( 'Parent', 'mtekk-post-parents' ), array(
					$this,
					'parent_meta_box'
				), $post_type->name, 'side', 'default' );
			}
		}
	}
...
function can_edit_acf_form(
	$post_id = 0, $user_id = 0, $allowed_statuses = array(
	'draft',
	'pending',
	'archive',
	'reviewed'
...
function mailchimp_pop_embed_js() {
	...
	<script type="text/javascript">require(["mojo/signup-forms/Loader"], function (L) {
			L.start({"baseUrl": "mc.us13.list-manage.com", "uuid": "57778163aacf3b56841696fe1", "lid": "8445d592ef"})
		})</script>
...
function opsi_admin_notice() {
	...
	if ( $query_opsi->post_count > 0 ) {
		...
		<div class="notice notice-warning is-dismissible">
			<p>
				<a href="<?php admin_url(); ?>edit.php?post_status=pending&post_type=case&case_type=opsi"><strong><?php echo sprintf( __( 'There are %d pending OPSI Case Studies', 'opsi' ), $query_opsi->post_count ); ?></strong></a>
			</p>
		</div>
		<?php
	}
	...
	if ( $query_opengov->post_count > 0 ) {
		...
		<div class="notice notice-warning is-dismissible">
			<p>
				<a href="<?php admin_url(); ?>edit.php?post_status=pending&post_type=case&case_type=open-government"><strong><?php echo sprintf( __( 'There are %d pending Open Government Case Studies', 'opsi' ), $query_opengov->post_count ); ?></strong></a>
			</p>
		</div>
		<?php
	}
	...
	if ( $users_count > 0 ) {
		...
		<div class="notice notice-warning is-dismissible">
			<p>
				<a href="<?php admin_url(); ?>users.php?role=pending"><strong><?php echo sprintf( __( 'There are %d Pending Users', 'opsi' ), $users_count ); ?></strong></a>
			</p>
		</div>
		<?php
	}
...
// force login for BI pre-registration project form and BI archives
add_action( 'template_redirect', 'bi_preregistration_redirect' );
function bi_preregistration_redirect() {
	if ( ( is_page('bi-pre-registration-form') || is_post_type_archive( 'bi-unit' ) || is_post_type_archive( 'bi-project' ) || is_singular( 'bi-unit' ) || is_singular( 'bi-project' ) ) && ! is_user_logged_in() ) {
...
function cs_on_publish_pending_post( $post ) {
	...
	if ( 'case' === get_post_type() ) { // check the custom post type
		...
		$subject = get_field( 'author_published_notification_subject', 'option' );
		$body    = str_replace( array( '%authorname%', '%casestudylink%', '%casestudytitle%' ), array(
			$author_fullname,
			$cslink,
			$cstitle
		), $mail_content );
...
add_filter( 'send_new_user_denial_notification', '__return_false', 20 );
